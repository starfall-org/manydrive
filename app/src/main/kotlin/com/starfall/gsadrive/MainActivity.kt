...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        model = model.copy(user = auth.currentUser?.displayName ?: auth.currentUser?.email, savedAccounts = accounts.accounts())
        setContent { ManyDriveTheme { App(model, tab, { selectTab(it) }, ::signIn, ::authorize, ::connectS3, ::signOut, { filePicker.launch(arrayOf("*/*")) }, ::createFolder, ::moveToTrash) } }
    }
    ...
    private fun signIn() {
        ...
        lifecycleScope.launch {
            runCatching {
                val option = GetGoogleIdOption.Builder().setServerClientId(getString(R.string.google_oauth_server_client_id))
                    .setFilterByAuthorizedAccounts(false).setAutoSelectEnabled(false).build()
                credentials.getCredential(this@MainActivity, GetCredentialRequest.Builder().addCredentialOption(option).build()).credential
                ...
                if (credential is CustomCredential && credential.type == GoogleIdTokenCredential.TYPE_GOOGLE_ID_TOKEN_CREDENTIAL) {
                    ...
                    auth.signInWithCredential(GoogleAuthProvider.getCredential(token, null)).addOnSuccessListener {
                        auth.currentUser?.email?.let(accounts::select)
                        model = model.copy(user = auth.currentUser?.displayName ?: auth.currentUser?.email, savedAccounts = accounts.accounts(), loading = false)
                        authorize()
    ...
    private fun authorize() {
        model = model.copy(loading = true, message = null)
        authorization.authorize(AuthorizationRequest.builder().setRequestedScopes(listOf(Scope(DRIVE_SCOPE), Scope(PHOTOS_SCOPE))).build())
            .addOnSuccessListener { result ->
                when {
                    result.accessToken != null -> loadDrive(result.accessToken!!)
                    result.hasResolution() -> {
                        model = model.copy(loading = false)
                        result.pendingIntent?.let { resolution.launch(IntentSenderRequest.Builder(it.intentSender).build()) } ?: error("Không thể mở cấp quyền.")
                    }
            ...
            runCatching {
                withContext(Dispatchers.IO) {
                    uris.forEach { uri ->
                        ...
                        val bytes = contentResolver.openInputStream(uri)?.use { it.readBytes() } ?: error("Không thể đọc $name")
    ...
    private fun signOut() { auth.signOut(); model = Model(message = "Đã đăng xuất.", savedAccounts = accounts.accounts()) }
    private fun error(text: String) { model = model.copy(loading = false, message = text) }
}
...
@Composable
private fun App(model: Model, selected: Int, select: (Int) -> Unit, signIn: () -> Unit, authorize: () -> Unit, connectS3: (S3Config) -> Unit, signOut: () -> Unit, upload: () -> Unit, createFolder: (String) -> Unit, trash: (DriveFile) -> Unit) {
    val tabs = listOf(Tab("Tệp", Icons.Outlined.Folder), Tab("Chia sẻ", Icons.Outlined.People), Tab("Ảnh", Icons.Outlined.Image), Tab("S3", Icons.Outlined.Cloud))
    Scaffold(
        topBar = { CenterAlignedTopAppBar(title = { Text("ManyDrive", fontWeight = FontWeight.SemiBold) }, actions = {
            IconButton(onClick = signIn) { Icon(Icons.Outlined.AccountCircle, "Tài khoản") }
            IconButton(onClick = { model.token?.let { if (selected == 2) select(2) else select(selected) } }, enabled = model.token != null) { Icon(Icons.Outlined.Refresh, "Làm mới") }
        }, colors = TopAppBarDefaults.centerAlignedTopAppBarColors(containerColor = MaterialTheme.colorScheme.surface)) },
        bottomBar = { NavigationBar { tabs.forEachIndexed { i, item -> NavigationBarItem(selected = i == selected, onClick = { select(i) }, icon = { Icon(item.icon, item.label) }, label = { Text(item.label) }) } } }
    ) { padding ->
        when (selected) {
...
@Composable
private fun DrivePage(model: Model, padding: PaddingValues, signIn: () -> Unit, authorize: () -> Unit, signOut: () -> Unit, upload: () -> Unit, createFolder: (String) -> Unit, trash: (DriveFile) -> Unit) {
    var folderName by remember { mutableStateOf("") }
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item { Surface(color = MaterialTheme.colorScheme.secondaryContainer, shape = MaterialTheme.shapes.large, modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(20.dp)) {
                Text(model.user?.let { "$it’s Drive" } ?: "Lưu trữ cùng Google Drive", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold)
                ...
                if (model.user == null) FilledTonalButton(onClick = signIn) { Text("Đăng nhập Google") }
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        FilledTonalButton(onClick = upload) { Icon(Icons.Outlined.UploadFile, null); Spacer(Modifier.width(6.dp)); Text("Tải tệp lên") }
                        FilledTonalButton(onClick = signOut) { Text("Đăng xuất") }
                    }
                    Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(top = 10.dp)) {
                        OutlinedTextField(folderName, { folderName = it }, label = { Text("Thư mục mới") }, singleLine = true, modifier = Modifier.weight(1f))
                        FilledTonalButton(onClick = { createFolder(folderName); folderName = "" }, enabled = folderName.isNotBlank(), modifier = Modifier.padding(start = 8.dp)) { Text("Tạo") }
                    }
                }
            }
        } }
...
private fun PhotosPage(model: Model, padding: PaddingValues, signIn: () -> Unit, authorize: () -> Unit) {
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item { Text("Google chỉ cho phép app đọc ảnh do chính app tạo. Để chọn ảnh bất kỳ, cần dùng Google Photos Picker.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
        ...
        if (model.loading) item { Loading() }
        items(model.photos, key = { it.id }) { photo -> Row(Modifier.fillMaxWidth().padding(vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(Icons.Outlined.Image, null, tint = MaterialTheme.colorScheme.primary); Spacer(Modifier.width(16.dp)); Column { Text(photo.filename); Text(photo.mimeType, style = MaterialTheme.typography.bodySmall) }
        } }
    }
}
...
private fun S3Page(model: Model, padding: PaddingValues, connect: (S3Config) -> Unit) {
    var endpoint by remember { mutableStateOf("") }; var key by remember { mutableStateOf("") }; var secret by remember { mutableStateOf("") }; var bucket by remember { mutableStateOf("") }; var region by remember { mutableStateOf("us-east-1") }
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        item { Text("S3 Compatible Storage", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.SemiBold) }
        item { Text("AWS S3, MinIO và storage S3-compatible. Secret key chỉ được giữ trong phiên hiện tại.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
        item { OutlinedTextField(endpoint, { endpoint = it }, label = { Text("Endpoint") }, modifier = Modifier.fillMaxWidth(), singleLine = true) }
        item { OutlinedTextField(key, { key = it }, label = { Text("Access key") }, modifier = Modifier.fillMaxWidth(), singleLine = true) }
        item { OutlinedTextField(secret, { secret = it }, label = { Text("Secret key") }, modifier = Modifier.fillMaxWidth(), singleLine = true) }
        item { OutlinedTextField(bucket, { bucket = it }, label = { Text("Bucket") }, modifier = Modifier.fillMaxWidth(), singleLine = true) }
        item { OutlinedTextField(region, { region = it }, label = { Text("Region") }, modifier = Modifier.fillMaxWidth(), singleLine = true) }
        item { FilledTonalButton(onClick = { connect(S3Config(endpoint, key, secret, bucket, region)) }, enabled = endpoint.isNotBlank() && key.isNotBlank() && secret.isNotBlank() && bucket.isNotBlank()) { Text("Kết nối S3") } }
        model.message?.let { item { Text(it, color = MaterialTheme.colorScheme.error) } }
        if (model.loading) item { Loading() }
...
@Composable private fun FileRow(file: DriveFile, onDelete: ((DriveFile) -> Unit)? = null) = Row(Modifier.fillMaxWidth().padding(vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) {
    Icon(if (file.isFolder) Icons.Outlined.Folder else Icons.Outlined.Description, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(28.dp)); Spacer(Modifier.width(16.dp))
    Column(Modifier.weight(1f)) { Text(file.name); Text(file.description, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
    onDelete?.let { IconButton(onClick = { it(file) }) { Icon(Icons.Outlined.Delete, "Chuyển vào thùng rác") } }
}
@Composable private fun Loading() = Box(Modifier.fillMaxWidth().padding(28.dp), contentAlignment = Alignment.Center) { CircularProgressIndicator() }
